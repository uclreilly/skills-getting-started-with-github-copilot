  ...
  async function fetchActivities() {
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        // Build participants section as a pretty bulleted list
        let participantsSection = `<div class="participants-section">
          <h5>Participants</h5>
          <ul class="participants-list">`;
        if (details.participants.length > 0) {
          details.participants.forEach(email => {
            participantsSection += `<li class="participant-item" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(email)}">
              <span class="participant-email">${email}</span>
              <span class="delete-participant" title="Remove participant">&#128465;</span>
            </li>`;
          });
        } else {
          participantsSection += '<li class="no-participants"><em>No participants yet</em></li>';
        }
        participantsSection += `</ul></div>`;
